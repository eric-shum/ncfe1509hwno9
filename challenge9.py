...
class challenge9(unittest.TestCase):
    ...
    alist = ["honda", "toyota", "nissan skyline", "nissan"]
    for l in alist:
        payload.update({"query": l})
        response = requests.request("POST", url, data=payload, headers=headers)
        response_dict = json.loads(response.text)
        size = response_dict['data']['query']['size']
        i = 0
        while i < size:
            print(' ')
            beginStr = l + " -- data->results->content->" + str(i) + "->"

            strList = ["lotNumberStr", "mkn", "lm", "fv", "egn", "cy", "ld", "yn", "cuc", "tz", "at", "bndc", "dd",
                       "tims", "gr", "dtc", "htsmn", "tmtp", "lmc", "lcc", "bstl", "lcd", "ft", "hk", "drv",
                       "ess", "syn", "brand", "hegn"]
            for item in strList:
                try:
                    theItem = response_dict['data']['results']['content'][i][item]
                    if isinstance(response_dict['data']['results']['content'][i][item], str):
                        print(beginStr + item + " is a string")
                    else:
                        print(beginStr + item + " is NOT a string --- FAILED")
                except:
                    print(beginStr + item + " does not exist")

            intfloatList = ["ln", "lcy", "la", "rc", "orr", "aan", "hb", "ss", "bnp", "ynumb", "phynumb", "ymin",
                            "myb", "crg"]
            for item in intfloatList:
                try:
                    theItem = response_dict['data']['results']['content'][i][item]
                    if isinstance(response_dict['data']['results']['content'][i][item], (float, int)):
                        print(beginStr + item + " is a float or an integer")
                    else:
                        print(beginStr + item + " is NOT a float or an integer --- FAILED")
                except:
                    print(beginStr + item + " does not exist")

            boolList = ["sbf", "bf", "offFlg", "showSeller", "sstpflg", "ifs", "pbf"]
            for item in boolList:
                try:
                    theItem = response_dict['data']['results']['content'][i][item]
                    if isinstance(response_dict['data']['results']['content'][i][item], bool):
                        print(beginStr + item + " is a boolean")
                    else:
                        print(beginStr + item + " is NOT a boolean --- FAILED")
                except:
                    print(beginStr + item + " does not exist")
            ...
            for item in theList:
                try:
                    theItem = response_dict['data']['results']['content'][i][item]
                    if isinstance(response_dict['data']['results']['content'][i][item], list):
                        print(beginStr + item + " is a list")
                    else:
                        print(beginStr + item + " is NOT a list --- FAILED")
                except:
                    print(beginStr + item + " does not exist")
